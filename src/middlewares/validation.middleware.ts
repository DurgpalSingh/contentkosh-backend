...
export const authorizeUserAccess = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        ...
        const { findPublicById } = await import('../repositories/user.repo');
        const targetUser = await findPublicById(userId);

        if (!targetUser) return ApiResponseHandler.notFound(res, 'User not found');

        const isSuperAdmin = req.user?.role === UserRole.SUPERADMIN;
        const hasBusinessAccess = targetUser.businessId === req.user?.businessId;

        if (!isSuperAdmin && !hasBusinessAccess) {
            return ApiResponseHandler.error(res, 'Forbidden: You do not have access to this user', 403);
        }
